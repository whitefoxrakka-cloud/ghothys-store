...
async function handleAdminRoute(request, env, url, headers) {
	...
	/* Status sesi untuk panel owner di halaman toko.
	   ...
	   WAJIB dicek SEBELUM gerbang token di bawah. */
	if (path === '/admin/session' && method === 'GET') {
		const bearerSesi = (request.headers.get('Authorization') || '').trim().replace(/^Bearer\s+/i, '').trim();
		const sesi = await verifyJwt(env.JWT_SECRET, bearerSesi || tokenDariCookie(request));
